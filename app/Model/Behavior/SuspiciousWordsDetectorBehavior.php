...
class SuspiciousWordsDetectorBehavior extends ModelBehavior
{
    ...
    function beforeSave(Model $Model)
    {
        ...
        if (!empty($this->__settings[$Model->alias]['fields']) && $this->__badWords) {
            ...
            $suspend_array = array(
                'Contest' => Configure::read('suspicious_detector.auto_suspend_contest_on_system_flag') ,
                'MessageContent' => Configure::read('suspicious_detector.auto_suspend_message_on_system_flag') ,
                'ContestUser' => Configure::read('suspicious_detector.auto_suspend_contest_user_on_system_flag') ,
            );
            if (!empty($is_system_flagged)) {
                   if (Configure::read('suspicious_detector.is_enabled')==1) {// During auto suspend turned on by admin
                    $Model->data[$Model->alias]['is_system_flagged'] = 1;
                       if ($suspend_array[$Model->alias]){
