  ...
  <div class="container-fluid space sep-bot">
	...
	<?php $class = (!empty($this->request->params['named']['filter_id']) && $this->request->params['named']['filter_id'] == ConstUploadStatus::Success) ? 'pinkc' : 'grayc'; ?>
		...
		<li class="span dc no-mar">
          <?php echo $this->Html->link('<div class="span"> <span class="label label-important span1 dc space no-mar"><i class="icon-thumbs-up no-pad text-24 whitec"></i></span> <span class="show  '.$class.' ">' . __l('Success').'</span></div><span class="'.$count_class.' text-32 textb space ">'.$this->Html->cInt($success, false).'</span>', array('controller' => 'audio_uploads', 'action' => 'index', 'filter_id' => ConstUploadStatus::Success),array('escape' => false, 'class'=>"blackc")); ?></li>
         ...
         <li class="span dc no-mar">
           <?php echo $this->Html->link('<div class="span"> <span class="label label-important span1 dc space no-mar"><i class="icon-spinner no-pad text-24 whitec"></i></span> <span class="show  '.$class.' ">' . __l('Processing').'</span></div><span class="'.$count_class.' text-32 textb space ">'.$this->Html->cInt($processing, false).'</span>', array('controller' => 'audio_uploads', 'action' => 'index', 'filter_id' => ConstUploadStatus::Processing),array('escape' => false, 'class'=>"blackc")); ?></li>
		 ...
		 <li class="span dc no-mar">
           <?php echo $this->Html->link('<div class="span"> <span class="label label-important span1 dc space no-mar"><i class="icon-thumbs-down no-pad text-24 whitec"></i></span> <span class="show  '.$class.' ">' . __l('Failure').'</span></div><span class="'.$count_class.' text-32 textb space ">'.$this->Html->cInt($failure, false).'</span>', array('controller' => 'audio_uploads', 'action' => 'index', 'filter_id' => ConstUploadStatus::Failure),array('escape' => false, 'class'=>"blackc")); ?></li>
...
<table class="table table-striped table-hover">
	<thead class="yellow-bg">
		<tr>      
			...
			<th class="actions"><?php echo $this->Paginator->sort('AudioUploadService.name', __l('Hoster'));?></th>
...
?>
	<tr>
		<td class="dl"><?php echo $this->Html->cDateTimeHighlight($upload['AudioUpload']['created']);?></td>
		<td class="dl">
		...
		<td class="dl"><?php echo $this->Html->cText($upload['AudioUploadService']['name']);?><?php echo '['.$this->Html->cText($upload['UploadServiceType']['name']).']';?></td>	
		<td class="dl">
		...
		<td class="dl">
			...
			if(!empty($upload['AudioUpload']['failure_message'])) { ?>
		        <span class="info chart-info js-tooltip hoster-info" title="<?php echo $this->Html->cText($upload['AudioUpload']['failure_message'], false);?>"></span>
				<?php 
			} 
			if($upload['AudioUpload']['upload_status_id'] == ConstUploadStatus::Processing) {
				echo "  " .  $this->Html->link(__l('Check Status'), array('controller'=>'audio_uploads','action'=>'check_status', $upload['AudioUpload']['id']),array('title' => __l('Click here to check upload status'), 'class'=>'js-tooltip'));
			} ?> 
        </td>
	</tr>
<?php
...
?>
	<tr>
		<td colspan="5" class="notice"><?php echo sprintf(__l('No %s available'), __l('audio uploads'));?></td>
	</tr>
<?php
endif;
?>
