  ...
  <div class="row space bot-mspace">
	...
	<div class="span dc pull-right  top-mspace">
	    <span class="hor-mspace">
			<?php echo $this->Html->link('<span><i class="icon-cog"></i></span> <span class="pinkc">' . __l('Settings') . '</span>', array('controller' => 'settings', 'action' => 'plugin_settings', 'AudioResources'), array('class' => 'grayc','title'=>__l('Settings'),'escape' => false)); ?>
        </span>
	</div>
  </div>
...
?>
<tr <?php if(empty($uploadHoster['AudioUploadHoster']['is_active'])){ echo 'class="disabled"';} ?>>
		<td  class="actions">
        ...
        <a href="#" data-toggle="dropdown" class="dropdown-toggle"><i class="icon-cog greenc text-20 blackc"></i></a>
            <ul class="dropdown-menu dl arrow">									
                <li><?php echo $this->Html->link('<i class="icon-cog"></i>' . __l('Configuration'), array('controller' => 'audio_upload_hosters', 'action' => 'edit', $uploadHoster['AudioUploadHoster']['upload_service_id']), array('class' => 'edit js-edit', 'title' => __l('Configuration'), 'escape' => false));
							?>
			 </li>
			  <li>
			<?php echo $this->Html->link(($uploadHoster['AudioUploadHoster']['is_active'] ==1)? '<i class="icon-eye-close"></i>' . __l('Inactive'): '<i class="icon-eye-open"></i>' . __l('Active'), array('controller' => 'audio_upload_hosters', 'action'=>'update_status', $uploadHoster['AudioUploadHoster']['id'],'status'=>($uploadHoster['AudioUploadHoster']['is_active'] ==1)? 'inactive': 'active'), array('class' => ($uploadHoster['AudioUploadHoster']['is_active'] ==1)? 'js-confirm js-no-pjax reject': 'js-confirm js-no-pjax active-link', 'title' => ($uploadHoster['AudioUploadHoster']['is_active'] ==1)? __l('Inactive'): __l('Active'), 'escape' => false));?>
			 </li>      
            </ul>
            </div>
		 </td>
		<td class="dl"><?php echo $this->Html->cText($uploadHoster['AudioUploadService']['name']);?> <span class="label label-default"><?php echo $this->Html->cText($uploadHoster['UploadServiceType']['name']);?></span>
		<?php if($uploadHoster['AudioUploadService']['id'] == ConstUploadService::YouTube): ?>
		...
		<?php if($uploadHoster['AudioUploadService']['id'] == ConstUploadService::Vimeo && $uploadHoster['UploadServiceType']['id'] == ConstUploadServiceType::Direct): ?>
		...
		<?php if($uploadHoster['AudioUploadService']['id'] == ConstUploadService::YouTube && $uploadHoster['UploadServiceType']['id'] == ConstUploadServiceType::Direct): ?>
		...
		<td class="dl"><?php echo $this->Html->cText($uploadHoster['AudioUploadService']['name']);?></td>	
		<td class="dl">
		<?php if($uploadHoster['AudioUploadService']['id'] == ConstUploadService::SoundCloud): ?>
				<?php echo configure::read('soundcloud_username');?>
		<?php endif; ?>
		</td>
		<?php if($b%2 == 1) { ?>
        <td rowspan="2" class="dc"> 
				<?php if($uploadHoster['AudioUploadService']['id'] == ConstUploadService::Vimeo) { ?>
 					<?php
 						$upload_progress_precentage = 0;
 						$finished_class = '';
 						if ($uploadHoster['AudioUploadService']['total_upload_filesize'] > 0 && $uploadHoster['AudioUploadService']['total_quota'] > 0) {
 							$upload_progress_precentage = round(($uploadHoster['AudioUploadService']['total_upload_filesize'] / $uploadHoster['AudioUploadService']['total_quota']*100), 2);
 						}
 					...
 					<p class="progress-value clearfix">
 						<div><?php echo $this->Html->cInt(($uploadHoster['AudioUploadService']['total_upload_filesize']/(1024*1024*1024))) . " " . __l("GB");?>  / <?php echo $this->Html->cInt(($uploadHoster['AudioUploadService']['total_quota']/(1024*1024*1024))) . " " . __l("GB");?></div>
 					</p>
 					</div>
 				<?php } else {
 						echo __l('N/A');	
 				...
 				<?php } ?>
		<td class="dl">
        	<?php echo $this->Html->link($this->Html->cInt($uploadHoster['AudioUploadHoster']['total_upload_count'], false), array('controller' => 'audio_uploads', 'action' => 'index', 'upload_service_id' => $uploadHoster['AudioUploadService']['id'], 'upload_service_type_id' => $uploadHoster['UploadServiceType']['id']));?>        
        </td>
		<td class="dl"><?php echo $this->Html->cInt($uploadHoster['AudioUploadHoster']['total_upload_filesize']);?></td>
        <?php $b++; ?>
	</tr>
<?php
    endforeach;
