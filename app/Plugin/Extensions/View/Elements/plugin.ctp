...
<?php $image_plugin_icons = array(
		...
		if (!in_array($pluginData['icon'], $image_plugin_icons)):
			echo $this->Html->image('plugin-icons/' . $pluginData['icon'] . '.png');
		else :
	  ...
	  <div class="row-fluid">
		<div class="pull-right">
		  <div class="btn-group">
			...
			<button data-toggle="dropdown" class="btn dropdown-toggle js-no-pjax"><span class="ver-mspace"><i class="icon-cog plugin-icon"></i></span><span class="caret"></span></button>
			<ul class="dropdown-menu arrow arrow-right pull-right">
			  ...
			  <?php } ?>
			  <?php if (!empty($pluginData['settings']) && $pluginData['active'] && filter_var($pluginData['settings'], FILTER_VALIDATE_BOOLEAN)) { ?>
			   <?php if ($pluginData['plugin_folder_name'] == 'VideoResources') { ?>
                    <li><?php echo $this->Html->link('<i class="icon-cog"></i>' . __l('Settings'), array('controller' => 'upload_hosters', 'action' => 'index'), array('escape'=>false,'title' => __l('Settings'))); ?></li>
                <?php } elseif($pluginData['name'] == ConstPaymentGatewaysName::SudoPay) { ?>
				...
				<?php } elseif($pluginData['plugin_folder_name'] == 'AudioResources') { ?>
					<li><?php echo $this->Html->link('<i class="icon-cog"></i>' . __l('Settings'), array('controller' => 'audio_upload_hosters', 'action' => 'index'), array('escape'=>false,'title' => __l('Settings')));?></li>
				<?php } else { ?>
