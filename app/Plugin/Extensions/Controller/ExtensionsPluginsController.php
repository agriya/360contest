...
class ExtensionsPluginsController extends AppController
{
    ...
    public function admin_index() 
    {
		...
		$title_font_icon = array(
			'Users' => 'icon-user',
			'Core' => 'icon-th',
		);
        $title_description = array(
            'Modules' => __l('Modules you have purchased in our customer area is listed here.'),
		...
		$resources = "";
		if(!in_array('ImageResources', $pluginAliases)) {
			$resources .= "Image";
		}
		if(!in_array('VideoResources', $pluginAliases)) {
			$resources .= !empty($resources) ? ", Video" : "Video";
		}
		if(!in_array('AudioResources', $pluginAliases)) {
			$resources .= !empty($resources) ? ", Audio" : "Audio";
		}
		if(!in_array('TextResources', $pluginAliases)) {
			$resources .= !empty($resources) ? ", Text" : "Text";
		}
		if(!empty($resources)) {	
			$title_description['Resources'] = sprintf(__l('Additional %s resources can be bought from Agriya as plugins.'), $resources);
		}
        $this->set('title_font_icon', $title_font_icon);
