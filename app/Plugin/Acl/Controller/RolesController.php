...
class RolesController extends AppController
{
    ...
    function admin_add($parent_id = null)
    {
        ...
        if (!empty($parent_id)) {
            ...
            $this->pageTitle.= sprintf(__l(' | %s') , $parentRole['Role']['name']);
    ...
    function admin_edit($id = null)
    {
        ...
        if (!empty($this->request->data)) {
            if ($this->Role->save($this->request->data)) {
                $this->Session->setFlash(sprintf(__l('%s has been updated'), __l('User Type')) , 'default', null, 'success');
            } else {
                $this->Session->setFlash(sprintf(__l('%s could not be updated. Please, try again.'), __l('User Type')) , 'default', null, 'error');
            }
        } else {
    ...
    function admin_delete($id = null)
    {
        ...
        if ($this->Role->delete($id)) {
            $this->Session->setFlash(sprintf(__l('%s deleted'), __l('User Type')) , 'default', null, 'success');
            $this->redirect(array(
                'action' => 'index'
            ));
        } else {
