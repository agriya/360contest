...
<?php
	if(!empty($contestUser['TextResource'][0]['MessageContent']) && !empty($contestUser['TextResource'][0]['MessageContent']['text_resource'])):
		if (empty($page)) {
			$page = 1;
		}
		$avg_rating = 0;
...
<?php
	if ($contestUser['ContestUser']['contest_user_status_id'] == ConstContestUserStatus::Withdrawn) {
		...
		if ($contestUser['ContestUser']['contest_user_status_id'] == ConstContestUserStatus::Won){
			$text_entry = $this->HTML->ctext($contestUser['TextResource'][0]['MessageContent']['text_resource']);
		}else{
			$text_entry = $this->HTML->ctext(substr($contestUser['TextResource'][0]['MessageContent']['text_resource'], 0, round(strlen($contestUser['TextResource'][0]['MessageContent']['text_resource'])/2)));
		}
		?>
        <p class="contest-content no-mar textb">
        <?php
		echo $this->Html->link($text_entry, array('controller' => 'contest_users', 'action' => 'view', $contestUser['Contest']['slug'],  'entry' => $contestUser['ContestUser']['entry_no'], 'page'=>$page, 'admin'=> false),array('escape' => false, 'class' => 'js-entry text-resource grayc no-under'));  
		?>
        </p>
        <?php  
