...
class GoogleAnalyticsController extends AppController
{
    ...
    public function admin_form_bounce_chart() 
    {
        ...
        $this->loadModel('Contests.Resource');
        $resources = $this->Resource->find('all', array(
            'recursive' => -1
        ));
        $this->set('resources', $resources);
        foreach($resources as $resource) {
            $type_name = $resource['Resource']['id'];
            $contest_add_form_bounce_arr = array();
            $contest_fund_form_bounce_arr = array();
            // contest add form bounce query
            $stats = $this->_getStats($condition, 'ga:totalEvents', array(
                'dimensions' => 'ga:eventLabel',
                'sort' => 'ga:eventLabel',
                'filters' => 'ga:eventCategory=~Contest;ga:eventAction=~Contest' . $type_name . 'Posted;ga:eventLabel=~Step'
            ));
            $contest_add_steps_arr = array();
            $contest_add_events_arr = array();
            if (!empty($stats->rows)) {
                foreach($stats->rows as $stat) {
                    $contest_add_steps_arr[] = "'" . $stat[0] . "'";
                    $contest_add_events_arr[] = $stat[1];
                }
            }
            $contest_add_form_bounce_arr[] = '["Form", ' . implode(', ', $contest_add_steps_arr) . ']';
            $contest_add_form_bounce_arr[] = '["'. $resource['Resource']['name'] .' Contest Posted", ' . implode(', ', $contest_add_events_arr) . ']';
            $this->set('contest_add_form_bounces' . $type_name, implode(',', $contest_add_form_bounce_arr));
        }
        // contest user form bounce query
        $stats = $this->_getStats($condition, 'ga:totalEvents', array(
            'dimensions' => 'ga:eventLabel',
            'sort' => 'ga:eventLabel',
            'filters' => 'ga:eventCategory=~EntryPost;ga:eventAction=~Entry;ga:eventLabel=~Step '
        ));
        $entry_post_events_arr = array();
        $entry_post_steps_arr = array();
		$entry_post_form_bounce_arr = array();
        if (!empty($stats->rows)) {
            foreach($stats->rows as $stat) {
                $entry_post_steps_arr[] = "'" . $stat[0] . "'";
                $entry_post_events_arr[] = $stat[1];
            }
            if (!empty($entry_post_events_arr)) {
                $entry_post_form_bounce_arr[] = '["Form", ' . implode(', ', $entry_post_steps_arr) . ']';
                $entry_post_form_bounce_arr[] = '["Entry Post", ' . implode(', ', $entry_post_events_arr) . ']';
            }
        }
        $this->set('entry_post_form_bounces', implode(',', $entry_post_form_bounce_arr));
    }
