        ...
        <div class="row-fluid ver-space">
          ...
          <div class="span24">
			<?php
				...
				$i++;
				$type_name = $resource['Resource']['name'];
				$type_id = $resource['Resource']['id'];
			?>

				<section class="span12 no-mar dc sep-bot space <?php if(($i % 2) == 1) { echo 'sep-right'; } ?>">
				<h3 class="ver-mspace dl"><?php echo __l($type_name . ' Contest Post Form Bounces'); ?></h3>
				  <script type="text/javascript">
					  ...
					  <?php } ?>
						var data = google.visualization.arrayToDataTable([
						  <?php echo ${'contest_add_form_bounces'.$type_id}; ?>
						]);
						var chart = new google.visualization.BarChart(document.getElementById('form_bounces_chart_contest_post_<?php echo $type_id; ?>'));
						chart.draw(data);
					  <?php if(empty($this->request->params['isAjax'])) { ?>
					  }
					  <?php } ?>
					</script>
				  <div class="dc" id="form_bounces_chart_contest_post_<?php echo $type_id; ?>" style="width: 450px; height: 400px;"></div>
				</section>
			<?php } ?>

            <section class="span12 no-mar dc space">
			  <h3 class="ver-mspace dl"><?php echo __l('Entry Post Form Bounces'); ?></h3>
              <script type="text/javascript">
                  <?php if(empty($this->request->params['isAjax'])) { ?>
				  google.setOnLoadCallback(drawChart);
                  function drawChart() {
				  <?php } ?>
                    var data = google.visualization.arrayToDataTable([
					  <?php echo ${'entry_post_form_bounces'}; ?>
                    ]);
                    var chart = new google.visualization.BarChart(document.getElementById('form_bounces_chart_entry_post'));
                    chart.draw(data);
                  <?php if(empty($this->request->params['isAjax'])) { ?>
                  }
				  <?php } ?>
                </script>
              <div class="dc" id="form_bounces_chart_entry_post" style="width: 450px; height: 400px;"></div>
            </section>
          </div>
        </div>
      </div>
