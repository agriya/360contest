 ...
 */
class FormFieldsController extends ContestsAppController
{
    ...
    public function admin_add($formId = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->FormField->save($this->request->data)) {
                if ($this->RequestHandler->isAjax()) {
                    ...
                    $this->redirect(array(
                        ...
                        'action' => 'edit',
                        $this->request->data['FormField']['contest_type_id'],
                        'type' => 'form_fields'
                    ));
                }
            } else {
            ...
            $this->request->data['FormField']['contest_type_id'] = $formId;
            $types = $this->FormField->types;
            $this->set('types', $types);
        }
    ...
    public function admin_get_row($id)
    {
        ...
        $key = $this->FormField->find('count', array(
            'conditions' => array(
                'contest_type_id' => $field['contest_type_id']
            ),
			'recursive' => -1
        ));
    ...
    public function admin_delete($id = null)
    {
        ...
        if ($this->FormField->delete($id)) {
            $this->Session->setFlash(sprintf(__l('%s deleted'), __l('FormField')));
            $this->redirect(array(
                'controller' => 'contest_types',
                'action' => 'index'
            ));
