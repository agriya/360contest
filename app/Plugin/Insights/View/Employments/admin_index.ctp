...
<div class="userEmployments index">
  ...
  <?php $class = (!empty($this->request->params['named']['filter_id']) && $this->request->params['named']['filter_id'] == ConstMoreAction::Active) ? 'pinkc' : 'grayc'; ?>
	...
	<?php echo $this->Html->link('<div class="span"> <span class="label label-important show dc space no-mar"><i class="icon-ok-circle no-pad text-24 whitec"></i></span><span class="show  '.$class.' ">' . __l('Active').'</span></div><span class="'.$count_class.' no-mar text-32 textb space span ">'.$this->Html->cInt($approved,false).'</span>', array('controller'=>'employments','action'=>'index','filter_id' => ConstMoreAction::Active), array('class' => 'blackc', 'escape' => false));?></li>
	...
	<?php echo $this->Html->link('<div class="span"> <span class="label label-important show dc space no-mar"><i class="icon-ban-circle no-pad text-24 whitec"></i></span><span class="show  '.$class.' ">' . __l('Inactive').'</span></div><span class="'.$count_class.' no-mar text-32 textb space span ">'.$this->Html->cInt($pending,false).'</span>', array('controller'=>'employments','action'=>'index','filter_id' => ConstMoreAction::Inactive), array('class' => 'blackc', 'escape' => false));?></li>    
   ...
   <?php echo $this->Html->link('<div class="span"> <span class="label label-important show dc space no-mar"><i class="icon-sitemap no-pad text-24 whitec"></i></span><span class="show  '.$class.' ">' . __l('All').'</span></div><span class="'.$count_class.' no-mar text-32 textb space span ">'.$this->Html->cInt($pending + $approved,false).'</span>', array('controller'=>'employments','action'=>'index'), array('class' => 'blackc', 'escape' => false));?></li>
    </ul>
	</div>
  </section>
    ...
    <table class="table table-striped table-bordered table-condensed table-hover no-mar">
      ...
      <tbody>
        ...
        <tr <?php echo $disabled; ?>>
          <td  class="select dc"><?php echo $this->Form->input('Employment.'.$userEmployment['Employment']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$userEmployment['Employment']['id'], 'label' => false, 'class' => $status_class.' js-checkbox-list')); ?></td>
          <td class="span1 dc">
            <div class="dropdown top-space">
              ...
              <ul class="unstyled dropdown-menu dl arrow clearfix">
                <li><?php echo $this->Html->link('<i class="icon-edit"></i>'.__l('Edit'), array( 'action'=>'edit', $userEmployment['Employment']['id']), array('class' => '','escape'=>false, 'title' => __l('Edit')));?></li>
                <li><?php echo $this->Html->link('<i class="icon-remove"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$userEmployment['Employment']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?></li>
                <?php echo $this->Layout->adminRowActions($userEmployment['Employment']['id']);  ?>
              </ul>
            </div>
          </td>
          <td class="dl"><?php echo $this->Html->cText($userEmployment['Employment']['employment']);?></td>
        </tr>
        ...
        ?>
        <tr>
          <td colspan="6" class="notice space"><i class="icon-warning-sign grayc"></i> <?php echo sprintf(__l('No %s available'), __l('Employment'));?></td>
        </tr>
        <?php
          endif;
        ?>
