...
<?php
	if ($contestUser['ContestUser']['contest_user_status_id'] == ConstContestUserStatus::Withdrawn) {
		...
		if ($link) {
			echo $this->Html->link($this->Html->showImage('ContestUser', $contestUser['Attachment'][0], array('dimension' => $dimension, 'alt' => sprintf(__l('[Image: %s]'), $contestUser['User']['username']), 'title' => $contestUser['User']['username'], 'escape' => false, 'class' => 'js-entry')), array('controller' => 'contest_users', 'action' => 'view', $contestUser['Contest']['slug'],  'entry' => $contestUser['ContestUser']['entry_no'], 'page'=>$page, 'admin'=> false), array('escape' => false ));
		} else {
			echo $this->Html->showImage('ContestUser', $contestUser['Attachment'][0], array('dimension' => $dimension, 'alt' => sprintf(__l('[Image: %s]'), $contestUser['User']['username']), 'title' => $contestUser['User']['username'], 'escape' => false, 'class' => 'js-entry' ));
		}
	}
?>
