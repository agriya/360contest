<?php if (Configure::read('site.launch_mode') == 'Private Beta') { ?>
  <?php //echo $this->Layout->sessionSubscriptionFlash(); ?>
  <?php if (!$success_msg): ?>
    <?php
...
<?php } else { ?>
  <?php if (!$success_msg): ?>
    ...
    <?php echo $this->Html->link(__l('Privacy Policy'), array('controller' => 'pages', 'action' => 'view', 'privacy', 'admin' => false), array('title' => __l('Privacy Policy'), 'class'=>'js-no-pjax', 'data-toggle' => 'modal', 'data-target' => '#js-ajax-modal'));?>
    </div>
