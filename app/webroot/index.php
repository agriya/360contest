...
if (!in_array($_SERVER['REQUEST_METHOD'], array(
	...
	if ((isset($_SERVER['HTTP_USER_AGENT']) && ((strtolower($_SERVER['HTTP_USER_AGENT']) == 'shockwave flash') || (strpos(strtolower($_SERVER['HTTP_USER_AGENT']) , 'adobe flash player') !== false))) && strpos($requested_url, 'flashupload') !== false) {
		$url_arr = explode('/', $requested_url);
		session_id($url_arr[3]);
		@session_start();
	}
