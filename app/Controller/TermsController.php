...
class TermsController extends AppController
{
    ...
    public function admin_index($vocabularyId = null)
    {
        ...
        if (!isset($vocabulary['Vocabulary']['id'])) {
            $this->Session->setFlash(__l('Invalid Vocabulary ID.') , 'default', null, 'error');
            $this->redirect(array(
                'controller' => 'vocabularies',
                'action' => 'index',
            ));
            ...
            if ($termId) {
                ...
                if ($termInVocabulary) {
                    ...
                    if ($this->Term->Taxonomy->save($taxonomy)) {
                        $this->Session->setFlash(__l('Term saved successfuly.') , 'default', null, 'success');
                        $this->redirect(array(
                            'action' => 'index',
                            $vocabularyId,
                        ));
                    } else {
                        $this->Session->setFlash(__l('Term could not be added to the vocabulary. Please try again.') , 'default', null, 'error');
                    }
                }
            } else {
                $this->Session->setFlash(__l('Term could not be saved. Please try again.') , 'default', null, 'error');
            }
        }
    ...
    {
        if (!$id || !$vocabularyId) {
            $this->Session->setFlash(sprintf(__l('Invalid id for %s'), __l('Term')) , 'default', null, 'error');
            $this->redirect(array(
                'action' => 'index',
                $vocabularyId,
            ));
