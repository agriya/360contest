...
class LinksController extends AppController
{
    ...
    public function admin_add($menuId = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->Link->save($this->request->data)) {
                $this->Session->setFlash(__l('Link has been saved') , 'default', null, 'success');
                $this->redirect(array(
                    'action' => 'index',
                    $this->request->data['Link']['menu_id']
                ));
            } else {
                $this->Session->setFlash(__l('Link could not be saved. Please, try again.') , 'default', null, 'error');
            }
        }
    ...
    public function admin_edit($id = null)
    {
        ...
        if (!$id && empty($this->request->data)) {
            $this->Session->setFlash(__l('Invalid Link') , 'default', null, 'error');
            $this->redirect(array(
                'controller' => 'menus',
                'action' => 'index',
            ));
    ...
    public function admin_delete($id = null)
    {
        ...
        if (!isset($link['Link']['id'])) {
            $this->Session->setFlash(sprintf(__l('Invalid id for %s'), __l('Link')) , 'default', null, 'error');
            $this->redirect(array(
                'controller' => 'menus',
                'action' => 'index',
            ));
    ...
    public function admin_process($menuId = null)
    {
        ...
        if ($action == 'delete' && $this->Link->deleteAll(array(
            ...
            $this->Session->setFlash(__l('Links deleted.') , 'default', null, 'success');
        } elseif ($action == 'publish' && $this->Link->updateAll(array(
            'Link.status' => 1
        ) , array(
            'Link.id' => $ids
        ))) {
            $this->Session->setFlash(__l('Links published') , 'default', null, 'success');
        } elseif ($action == 'unpublish' && $this->Link->updateAll(array(
            'Link.status' => 0
        ) , array(
            'Link.id' => $ids
        ))) {
            $this->Session->setFlash(__l('Links unpublished') , 'default', null, 'success');
        } else {
