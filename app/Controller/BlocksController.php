...
class BlocksController extends AppController
{
    ...
    public function admin_add()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->Block->save($this->request->data)) {
                $this->Session->setFlash(__l('Block has been saved') , 'default', null, 'success');
                ...
                $this->Session->setFlash(__l('Block could not be saved. Please, try again.') , 'default', null, 'error');
            }
        }
    ...
    public function admin_process()
    {
        ...
        if ($action == 'delete' && $this->Block->deleteAll(array(
            ...
            $this->Session->setFlash(__l('Blocks deleted.') , 'default', null, 'success');
        } elseif ($action == 'publish' && $this->Block->updateAll(array(
            'Block.status' => true
        ) , array(
            'Block.id' => $ids
        ))) {
            $this->Session->setFlash(__l('Blocks published') , 'default', null, 'success');
        } elseif ($action == 'unpublish' && $this->Block->updateAll(array(
            'Block.status' => false
        ) , array(
            'Block.id' => $ids
        ))) {
            $this->Session->setFlash(__l('Blocks unpublished') , 'default', null, 'success');
        } else {
