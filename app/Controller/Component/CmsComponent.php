...
class CmsComponent extends Component
{
    ...
    public function startup(Controller $controller)
    {
        ...
        $this->blocks();
        if (!empty($this->controller->request->params['action'])&& ($this->controller->request->params['action'] == 'admin_login' || $this->controller->request->params['action'] == 'do_payment')) {
            $this->menus();
        }
        ...
        foreach($menus as $menuAlias) {
            ...
            if (isset($menu['Menu']['id'])) {
                ...
                $findOptions = array(
                    ...
                    'cache' => array(
                        'name' => 'cms_menu_' . $menu['Menu']['id'] . '_links',
                        'config' => 'cms_menus',
                    ) ,
                    'recursive' => -1,
                );
