    ...
    public function add()
    {
        ...
        if (!empty($this->request->data['UserOpenid']['user_id'])) {
            ...
            if ($this->UserOpenid->save($this->request->data)) {
                $this->Session->setFlash(sprintf(__l('%s has been added'), __l('User Openid')) , 'default', null, 'success');
                $this->redirect(array(
                    'action' => 'index'
                ));
            } else {
                $this->Session->setFlash(sprintf(__l('%s could not be added. Please, try again.'), __l('User Openid')) , 'default', null, 'error');
            }
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->UserOpenid->find('count', array(
            ...
            if ($this->UserOpenid->delete($id)) {
                $this->Session->setFlash(sprintf(__l('%s deleted'), __l('User Openid')) , 'default', null, 'success');
                $this->redirect(array(
                    'action' => 'index'
                ));
            } else {
