...
class NodesController extends AppController
{
    ...
    public function admin_index()
    {
		...
		if (!empty($this->request->params['named']['content_filter_id']) && $this->request->params['named']['content_filter_id'] == constContentType::Page && !empty($this->request->params['named']['filter_id']) && $this->request->params['named']['filter_id'] == ConstMoreAction::Publish) {
			$publishCountConditions['Node.type'] = 'page';
		}
		...
		if (!empty($this->request->params['named']['content_filter_id']) && $this->request->params['named']['content_filter_id'] == constContentType::Page && !empty($this->request->params['named']['filter_id']) && $this->request->params['named']['filter_id'] == ConstMoreAction::Publish) {
			$unpublishCountConditions['Node.type'] = 'page';
		}
    ...
    public function admin_add($typeAlias = 'node')
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->Node->saveWithMeta($this->request->data)) {
                ...
                $this->Session->setFlash(sprintf(__l('%s has been saved') , $type['Type']['title']) , 'default', null, 'success');
                ...
                $this->Session->setFlash(sprintf(__l('%s could not be saved. Please, try again.') , $type['Type']['title']) , 'default', null, 'error');
            }
        } else {
    ...
    public function admin_edit($id = null)
    {
        if (!$id && empty($this->request->data)) {
            $this->Session->setFlash(__l('Invalid content') , 'default', null, 'error');
            $this->redirect(array(
                'action' => 'index'
            ));
        }
        ...
        $type = $this->Node->Taxonomy->Vocabulary->Type->findByAlias($typeAlias);
        if (!isset($type['Type']['alias'])) {
            $this->Session->setFlash(__l('Content type does not exist.') , 'default', null, 'error');
            $this->redirect(array(
                'action' => 'create'
            ));
        }
    ...
    public function admin_delete($id = null)
    {
        ...
        if ($this->Node->delete($id)) {
            $this->Session->setFlash(sprintf(__l('%s deleted'), __l('Node')) , 'default', null, 'success');
            $this->redirect(array(
                'action' => 'index'
            ));
        } else {
    ...
    public function admin_process()
    {
        ...
        if ($action == 'delete' && $this->Node->deleteAll(array(
            ...
            $this->Session->setFlash(__l('Nodes deleted.') , 'default', null, 'success');
            ...
            $this->Session->setFlash(__l('Nodes published') , 'default', null, 'success');
            ...
            $this->Session->setFlash(__l('Nodes unpublished') , 'default', null, 'success');
            ...
            $this->Session->setFlash(__l('Nodes promoted') , 'default', null, 'success');
            ...
            $this->Session->setFlash(__l('Nodes unpromoted') , 'default', null, 'success');
        } else {
    ...
    public function term()
    {
        ...
        if (!isset($term['Term']['id'])) {
            $this->Session->setFlash(__l('Invalid Term.') , 'default', null, 'error');
            $this->redirect('/');
        }
        if (!isset($this->request->params['named']['type'])) {
            $this->request->params['named']['type'] = 'node';
    ...
    public function view($id = null)
    {
		if (isset($this->request->params['named']['slug']) && isset($this->params['named']['type'])) {
            ...
            $node = $this->Node->find('first', array(
                ...
                'cache' => array(
                    'name' => 'node_' . $this->request->params['named']['type'] . '_' . $this->params['named']['slug'],
                    'config' => 'nodes_view',
                ) ,
				'recursive' => 2
            ));
	...
	public function admin_agriya_labs()
    {
        $this->pageTitle = __l('Agriys Labs Candidate');
    }
}
